//-----------------------------------------------------------------------------
//	Вывод звука посредством Direct Sound
...
typedef struct dx_device_list_s {
	...
	dx_device_s** _array;   			  // указатель на массив
} dx_device_list_t;


class CDXOutput : public CAbstractOutput {
public:
	static int _init;   			   // флаг инициализации
	static dx_device_list_t _deviceList;			// список устройств

	// внутренние методы
	static void Startup();  						// начало работы
	static void DeviceGrowArray();  				// инициализация списка устройств
	static int CreateDeviceList();  				// создание списка устройств
	//   static LPGUID GetDeviceID(int device); 		 // получение идентификатора устройства
	static dx_device_t* GetDevice(int device);  	// получение указателя на данные устройства
	static void Shutdown(); 						// окончание работы
	...
	virtual void Free();							//  Освобождение системных ресурсов занятых звуковым двигателем. 
	/*
	...
	*/
	...
	virtual int GetNumDevice(); 					// Получение количества устройств воспроизведения в системе.
	virtual char* GetDeviceName(int iDevice);   	// Получение имени устройства воспроизведения.
