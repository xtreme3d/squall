//-----------------------------------------------------------------------------
//	Абстрактный класс работы с каналами
//	Копонент звукового двигателя Шквал
//	команда		: AntiTank
//	разработчик	: Гилязетдинов Марат (Марыч)
//-----------------------------------------------------------------------------
#ifndef _CABSTRACT_CHANNEL_H_INCLUDED_
#define _CABSTRACT_CHANNEL_H_INCLUDED_
/*
// включения
#include "..\Squall.h"
#include <dsound.h>	
...
enum
{
   TYPE_POSITIONED = 1, 						   // звук в заданых координатах
...
struct SSample
{
   ...
   int Set3DParameters( D3DVECTOR *Position,
	  D3DVECTOR *Velocity,
	  D3DVECTOR *ConeOrientation,
	  unsigned int InsideConeAngle,
	  unsigned int OutsideConeAngle,
	  long ConeOutsideVolume,
	  float MinDist,
	  float MaxDist,
	  DWORD Deferred);
   // получение всех трехмерных параметров буфера
   int Get3DParameters( D3DVECTOR *Position,
	  D3DVECTOR *Velocity,
	  D3DVECTOR *ConeOrientation,
	  int* InsideConeAngle,
   ...
   int CreateSoundBuffer(LPDIRECTSOUND DirectSound,
	  WAVEFORMATEX* format,
	  DWORD Size,
	  DWORD SamplesPerBuffer,
	  int UseAcceleration);
   // создание трехмерного звукового буфера
   int Create3DSoundBuffer(LPDIRECTSOUND DirectSound,
	  WAVEFORMATEX* format,
	  DWORD Size,
	  DWORD SamplesPerBuffer,
	  DWORD Algorithm,
...
class CAbstractChannel {
	...
	CAbstractChannel();
	virtual ~CAbstractChannel() = 0;

	virtual int CreateSoundBuffer() = 0;
	virtual int Create3DSoundBuffer() = 0;
	...
	// 2D каналы
	virtual int SetPan() = 0;
	virtual int SetPan() = 0;

	// 3D каналы
	virtual int Set3DParameters() = 0;
	virtual int Get3DParameters() = 0;
};
#endif