...
BOOL CDecompressTrack::ReadMTM(LPCBYTE lpStream, DWORD dwMemLength)
	//-----------------------------------------------------------
{
	...
	if ((!lpStream) || (dwMemLength < 0x100))
		return FALSE;
	if ((strncmp(pmh->id, "MTM", 3)) ||
		(pmh->numchannels > 32) ||
		(pmh->numsamples >= MAX_SAMPLES) ||
		(!pmh->numsamples) ||
		(!pmh->numtracks) ||
		(!pmh->numchannels) ||
		(!pmh->lastpattern) ||
	...
	if (dwMemPos +
		...
		128 +
		192 * pmh->numtracks +
		64 * (pmh->lastpattern + 1) +
		pmh->commentsize >= dwMemLength)
		return FALSE;
	...
	for (UINT ismp = 1; ismp <= m_nSamples; ismp++) {
		...
		dwMemPos += ReadSample(&Ins[ismp],
						(Ins[ismp].uFlags & CHN_16BIT) ? RS_PCM16U : RS_PCM8U,
