	...
	UINT MusicTempo, UINT Position)
{
	...
	for (CurRow = Row; CurRow < PatternSize[Pattern]; CurRow++) {
		...
		// Если первый элемент в патерне, сохраним время начала первого элемента
		if (!CurRow) {
			for (UINT ipck = 0; ipck < m_nChannels; ipck++)
				patloop[ipck] = Samples;
		}

		// Вычисление указателя на данные элемента
		MODCHANNEL* pChn = Chn;
		...
		// Проверка на переход на следующий элемент
		if (BreakFlag != 0)
			break;
	}
	return -1;
...
void CDecompressTrack::CreateOrderInfo(void)
{
	...
	// Обработка списка патернов
	while (CurOrder < MAX_ORDERS && Order[CurOrder] != 0xFF) {
		SpeedCount = 0;

		// Получение номера текущего патерна из списка воспроизведения
		CurPattern = Order[CurOrder];
		NewOrder = CurOrder + 1;
		SampleOf = 0;

		// Проверка номера патерна
		if (CurPattern < MAX_PATTERNS) {
			// Проверка наличия патерна
			...
			// Запишем информацию о элементе
			if (OrderInfo[CurOrder]._init == 0) {
				OrderInfo[CurOrder]._init = 1;  				  // Флаг инициализации
				OrderInfo[CurOrder]._next = NewOrder - CurOrder;  // Смещение на следующий элемент в списке
				OrderInfo[CurOrder]._row = NewRow;  			 // Номер ров при переходе на следующий элемент
				OrderInfo[CurOrder]._samples = SampleOf;			 // Количество семплов в элементе
				OrderInfo[CurOrder]._speed = MusicSpeed;		   // Скорость
				OrderInfo[CurOrder]._tempo = MusicTempo;		   // Темп

				// Поиск максимального размера патерна в семплах
...
//----------------------------------------------------
{
   ...
   // Обработка списка патернов
	for (;;)
	{
	  ...
	  // Запишем информацию о патерне
	  if( OrderInfo[CurrentPattern]._init == 0 )
	  {
		 OrderInfo[CurrentPattern]._init		= 1;		   // Флаг инициализации
		 OrderInfo[CurrentPattern]._next		= 1;		   // Смещение на следующий элемент в списке
		 OrderInfo[CurrentPattern]._row 		= 0;		   // Номер ров при переходе на следующий элемент
		 OrderInfo[CurrentPattern]._samplePos   = SampleOf;    // Позиция 
		 OrderInfo[CurrentPattern]._speed   	= nMusicSpeed; // Скорость
		 OrderInfo[CurrentPattern]._tempo   	= nMusicTempo; // Темп
	  }

	  // Если номер воспроизводимого патерна больше допустимого
		while (nPattern >= MAX_PATTERNS)
		{
			...
			{
			// Сбросим патерн и возьмем новый патерн из списка
				CurrentPattern++;
		...
		// Если номер воспроизводимого патерна больше допустимого или такого патерна нет
	  // выход из цикла
		if ((nPattern >= MAX_PATTERNS) || (!Patterns[nPattern]))
		 break;

		// Если текущий элемент больше чем количество элементов в патерне, элемент равен 0
		if (nRow >= PatternSize[nPattern])
		 nRow = 0;

		// Вычисление следующего элемента
		nNextRow = nRow + 1;
	  // Если следующий элемент больше чем количество элементов в патерне,
	  // следующий элемент равен 0, также перейдем на следующий элемент списка
		 ...
		 if (*((DWORD *)p) && p->command )
		   {
			...
			switch (p->command)
			   ...
			   case CMD_POSITIONJUMP:
				  ...
				  goto EndMod;   // здесь происходит зацикливание 
				   nNextPattern = param;
				   nNextRow = 0;
				   break;
			   // Pattern Break
