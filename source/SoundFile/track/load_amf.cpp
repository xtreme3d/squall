	...
	while (nTrkSize--) {
		...
		} else if (cmd == 0x7F) // duplicate row
		{
			...
			if ((rowsrc >= 0) && (rowsrc < (int) nRows))
				memcpy(m,
					& pPat[rowsrc * nChannels],
					sizeof(pPat[rowsrc * nChannels]));
			...
			// effect
		{
			...
			case 0x04:
				if (param & 0x80) {
					command = CMD_PORTAMENTOUP; param = -(signed char) param;
				} else {
			...
			case 0x16:
				...
				// 0x13: Note Delay
			case 0x13:
				command = CMD_S3MCMDEX; param = 0xD0 | (param & 0x0F); break;
				// 0x14: Note Cut
			case 0x14:
				command = CMD_S3MCMDEX; param = 0xC0 | (param & 0x0F); break;
				// 0x15: Set Tempo
			case 0x15:
...
BOOL CDecompressTrack::ReadAMF(LPCBYTE lpStream, DWORD dwMemLength)
	//-----------------------------------------------------------
{
	...
	if ((!strncmp((LPCTSTR) lpStream, "ASYLUM Music Format V1.0", 25)) &&
		...
		for (UINT iOrd = 0; iOrd < MAX_ORDERS; iOrd++) {
			Order[iOrd] = (iOrd < numorders) ?
				lpStream[dwMemPos + iOrd] :
				0xFF;
		}
		dwMemPos = 294; // ???
		for (UINT iSmp = 0; iSmp < numsamples; iSmp++) {
			...
			psmp->nLoopStart = bswapLE32(*((LPDWORD)
				(lpStream + dwMemPos + 29)));
			psmp->nLoopEnd = psmp->nLoopStart +
				bswapLE32(*((LPDWORD) (lpStream + dwMemPos + 33)));
		...
		for (UINT iData = 0; iData < m_nSamples; iData++) {
			...
			if (psmp->nLength) {
				dwMemPos += ReadSample(psmp,
								RS_PCM8S,
								(LPCSTR) (lpStream + dwMemPos),
								dwMemLength);
			}
		}
		return TRUE;
	...
	if ((pfh->szAMF[0] != 'A') ||
		...
		(pfh->version < 10) ||
		(pfh->version > 14) ||
		(!bswapLE16(pfh->numtracks)) ||
		(!pfh->numorders) ||
		(pfh->numorders > MAX_PATTERNS) ||
		(!pfh->numsamples) ||
		(pfh->numsamples > MAX_SAMPLES) ||
		(pfh->numchannels < 4) ||
		(pfh->numchannels > 32))
		return FALSE;
	...
	if (pfh->version >= 11) {
		...
		for (UINT i = 0; i < nchannels; i++) {
			...
			if (pan < 0)
				pan = 0;
			if (pan > 256) {
				pan = 128; ChnSettings[i].dwFlags |= CHN_SURROUND;
			}
	...
	for (UINT iOrd = 0; iOrd < MAX_ORDERS; iOrd++) {
		...
		if (iOrd < pfh->numorders) {
			...
			PatternSize[iOrd] = 64;
			if (pfh->version >= 14) {
				PatternSize[iOrd] = bswapLE16(*(USHORT *)
										(lpStream + dwMemPos));
				dwMemPos += 2;
			}
