...
BOOL CDecompressTrack::ReadXM(const BYTE* lpStream, DWORD dwMemLength)
	//--------------------------------------------------------------
{
	XMSAMPLEHEADER xmsh;
	XMSAMPLESTRUCT xmss;
	...
	if ((!lpStream) || (dwMemLength < 0x200))
		return FALSE;
	if (_strnicmp((LPCSTR) lpStream, "Extended Module", 15))
		return FALSE;
			...
			for (UINT chn = 0; chn < m_nChannels; chn++) {
				if ((p) && (j < packsize)) {
					...
					if ((vol >= 0x10) && (vol <= 0x50)) {
						...
						case 0x60:
							p->volcmd = VOLCMD_VOLSLIDEDOWN; break;
							// 70-7F: Volume Slide Up:
						case 0x70:
							p->volcmd = VOLCMD_VOLSLIDEUP; break;
							// 80-8F: Fine Volume Slide Down
						case 0x80:
							p->volcmd = VOLCMD_FINEVOLDOWN; break;
							// 90-9F: Fine Volume Slide Up
						case 0x90:
							p->volcmd = VOLCMD_FINEVOLUP; break;
							// A0-AF: Set Vibrato Speed
						case 0xA0:
							p->volcmd = VOLCMD_VIBRATOSPEED; break;
							// B0-BF: Vibrato
						case 0xB0:
							p->volcmd = VOLCMD_VIBRATO; break;
							// C0-CF: Set Panning
						case 0xC0:
							p->volcmd = VOLCMD_PANNING; p->vol = (vol << 2) +
								2; break;
							// D0-DF: Panning Slide Left
						case 0xD0:
							p->volcmd = VOLCMD_PANSLIDELEFT; break;
							// E0-EF: Panning Slide Right
						case 0xE0:
							p->volcmd = VOLCMD_PANSLIDERIGHT; break;
							// F0-FF: Tone Portamento
						case 0xF0:
							p->volcmd = VOLCMD_TONEPORTAMENTO; break;
						}
					}
					p++;
				} else if (j < packsize) {
	...
	for (UINT iIns = 1; iIns <= instruments; iIns++) {
		...
		for (UINT ismpd = 0; ismpd < nsamples; ismpd++) {
			if ((samplemap[ismpd]) &&
				...
				ReadSample(&Ins[samplemap[ismpd]],
					flags[ismpd],
