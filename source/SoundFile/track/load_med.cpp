	...
	//---------------------------------------------------------------
{
	...
	case 0x0A:
		if (param & 0xF0)
			param &= 0xF0; command = CMD_VOLUMESLIDE; if (!param)
													  	command = 0; break;
	case 0x0B:
...
BOOL CDecompressTrack::ReadMed(const BYTE* lpStream, DWORD dwMemLength)
	//---------------------------------------------------------------
{
	...
	for (UINT iSHdr = 0; iSHdr < m_nSamples; iSHdr++) {
		...
		pins->nLoopEnd = pins->nLoopStart +
			(bswapBE16(pmsh->sample[iSHdr].replen) << 1);
			...
			for (UINT i = 0; i < m_nChannels; i++) {
				...
				if (trktagofs) {
					while (trktagofs + 8 < dwMemLength) {
						...
						if (ntag == MMDTAG_END)
							break;
						DWORD tagdata = bswapBE32(*(DWORD*)
											(lpStream + trktagofs + 4));
						switch (ntag) {
						case MMDTAG_TRK_NAMELEN:
							trknamelen = tagdata; break;
						case MMDTAG_TRK_NAME:
							trknameofs = tagdata; break;
						}
						trktagofs += 8;
					}
		...
		if (version == '0') {
			...
			if ((dwBlockInfo) &&
				...
				Log("  BLOCKINFO: blockname=0x%04X namelen=%d pagetable=0x%04X &cmdexttable=0x%04X\n",
					bswapBE32(pbi->blockname),
					bswapBE32(pbi->blocknamelen),
					bswapBE32(pbi->pagetable),
					bswapBE32(pbi->cmdexttable));
#endif
