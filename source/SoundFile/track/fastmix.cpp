...
CzCUBICSPLINE::CzCUBICSPLINE()
{
	...
	for (_LIi = 0; _LIi < _LLen; _LIi++) {
		...
		_LC0 = (float)
			floor(0.5 +
				_LScale * (1.5 * _LX * _LX * _LX - 2.5 * _LX * _LX + 1.0));
		_LC1 = (float)
			floor(0.5 +
				_LScale * (-1.5 * _LX * _LX * _LX + 2.0 * _LX * _LX +
			0.5 * _LX));
		_LC2 = (float) floor(0.5 +
					   	_LScale * (0.5 * _LX * _LX * _LX - 0.5 * _LX * _LX));
		lut[_LIdx + 0] = (signed short)
			((_LCm1 < -_LScale) ? -_LScale : ((_LCm1 > _LScale) ? _LScale : _LCm1));
		lut[_LIdx + 1] = (signed short)
			((_LC0 < -_LScale) ? -_LScale : ((_LC0 > _LScale) ? _LScale : _LC0));
		lut[_LIdx + 2] = (signed short)
			((_LC1 < -_LScale) ? -_LScale : ((_LC1 > _LScale) ? _LScale : _LC1));
		lut[_LIdx + 3] = (signed short)
			((_LC2 < -_LScale) ? -_LScale : ((_LC2 > _LScale) ? _LScale : _LC2));
...
CzWINDOWEDFIR::CzWINDOWEDFIR()
{
	...
	for (_LPcl = 0; _LPcl < WFIR_LUTLEN; _LPcl++) {
		...
		for (_LCc = 0; _LCc < WFIR_WIDTH; _LCc++) {
			float _LCoef = (float)
				floor(0.5 + _LScale* _LCoefs[_LCc] * _LGain);
			lut[_LIdx + _LCc] = (signed short)
				((_LCoef < -_LScale) ? -_LScale : ((_LCoef > _LScale) ? _LScale : _LCoef));
		}
	}
}
...
SNDMIX_BEGINSAMPLELOOP8
SNDMIX_GETMONOVOL8NOIDO
SNDMIX_STOREMONOVOL END_MIX_INTERFACE()
...
SNDMIX_BEGINSAMPLELOOP8
SNDMIX_GETMONOVOL8NOIDO
SNDMIX_RAMPMONOVOL END_RAMPMIX_INTERFACE()
	...
	//---------------------------------------------------------------------
{
	...
	if ((LONG) pChn->nPos < nLoopStart) {
		if (nInc < 0) {
			...
			LONG nDelta = ((nLoopStart - pChn->nPos) << 16) -
				(pChn->nPosLo & 0xffff);
	...
	if (nInc < 0) {
		...
		if (nPosDest < nLoopStart) {
			nSmpCount = (ULONG)
				(((((LONGLONG) nPos - nLoopStart) << 16) + nPosLo - 1) / nInv) +
				1;
		}
	} else {
		...
		if (nPosDest >= (LONG) pChn->nLength) {
			nSmpCount = (ULONG)
				(((((LONGLONG) pChn->nLength - nPos) << 16) - nPosLo - 1) /
				nInc) +
				1;
		}
	}
...
UINT CDecompressTrack::CreateStereoMix(int count)
	//-----------------------------------------
{
	LPLONG pOfsL, pOfsR;
	...
	for (UINT nChn = 0; nChn < m_nMixChannels; nChn++) {
		...
		if (!pChannel->pCurrentSample)
			continue;
		nMasterCh = (ChnMix[nChn] < m_nChannels) ?
			ChnMix[nChn] +
			1 :
			pChannel->nMasterChn;
		pOfsR = &gnDryROfsVol;
		...
		if (((nchmixed >= 32) && (!(gdwSoundSetup & SNDMIX_DIRECTTODISK))) ||
			((!pChannel->nRampLength) &&
			(!(pChannel->nLeftVol | pChannel->nRightVol)))) {
			LONG delta = (pChannel->nInc * (LONG) nSmpCount) +
...
DWORD MPPASMCALL X86_Convert32To16(LPVOID lp16, int* pBuffer,
	DWORD lSampleCount, LPLONG lpMin, LPLONG lpMax)
{
...
// на входе    :  pMixBuf  -  указатель на буфер с даными
//  			  nSamples -  количество семплов
// на выходе   :  *
//-----------------------------------------------------------------------------
VOID MPPASMCALL X86_MonoFromStereo(int* pMixBuf, UINT nSamples)
{
...
void CDecompressTrack::ProcessAGC(int count)
//------------------------------------
{
...
void CDecompressTrack::ResetAGC()
//-------------------------
{
	gnAGC = AGC_UNITY;
}
