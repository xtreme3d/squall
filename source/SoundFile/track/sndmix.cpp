...
//UINT CDecompressTrack::m_nStereoSeparation = 128;
//LONG CDecompressTrack::m_nStreamVolume = 0x8000;
//UINT CDecompressTrack::m_nMaxMixChannels = 32;
// Mixing Configuration (SetWaveConfig)
//DWORD CDecompressTrack::gdwSysInfo = 0;
DWORD CDecompressTrack::gnChannels = 2;
DWORD CDecompressTrack::gdwSoundSetup = 0;
DWORD CDecompressTrack::gdwMixingFreq = 44100;
DWORD CDecompressTrack::gnBitsPerSample = 16;
// Mixing data initialized in
UINT CDecompressTrack::gnAGC = AGC_UNITY;
UINT CDecompressTrack::gnVolumeRampSamples = 64;
UINT CDecompressTrack::gnVUMeter = 0;
UINT CDecompressTrack::gnCPUUsage = 0;
//LPSNDMIXHOOKPROC CDecompressTrack::gpSndMixHook = NULL;
//PMIXPLUGINCREATEPROC CDecompressTrack::gpMixPluginCreateProc = NULL;
LONG gnDryROfsVol = 0;
LONG gnDryLOfsVol = 0;
//LONG gnRvbROfsVol = 0;
LONG gnRvbLOfsVol = 0;
...
const UINT PreAmpAGCTable[16] = {
	0x60, 0x60, 0x60, 0x60, 0x68, 0x70, 0x78, 0x80, 0x84, 0x88, 0x8C, 0x90,
	0x94, 0x98, 0x9C, 0xA0, 
};
...
/*
BOOL CDecompressTrack::InitPlayer(BOOL bReset)
//--------------------------------------
{
...
*/
/*
BOOL CDecompressTrack::FadeSong(UINT msec)
//----------------------------------
{
...
/*
BOOL CDecompressTrack::GlobalFadeSong(UINT msec)
//----------------------------------------
{
...
UINT CDecompressTrack::Read(LPVOID lpDestBuffer, UINT cbBuffer,
	int out_channels)
	//-------------------------------------------------------
{
	LPBYTE lpBuffer = (LPBYTE) lpDestBuffer;
	//	LPCONVERTPROC pCvt = X86_Convert32To16;//X86_Convert32To8;
	...
	// Обработка плучинов
	 {
		...
		while ((nMaxPlugins > 0) &&
			(!m_MixPlugins[nMaxPlugins - 1].pMixPlugin))
			nMaxPlugins--;
	}
	m_nMixStat = 0;
	//	lSampleSize = gnChannels;
	lSampleSize = out_channels;   // размер семпла в байтах (соответсвует количеству каналов)

	if (gnBitsPerSample == 16) {
		lSampleSize *= 2;   	   // домножаем на размер одной выборки
		//  	pCvt = X86_Convert32To16;
	}

	lMax = cbBuffer / lSampleSize;   // вычисление количества семплов в буфере

	// если количество семплов равно 0 или буфер или количестов каналов то выход
	...
	// обрабатывать пока не прочитаем все нужные данные
	while (lRead > 0) {
		// Update Channel Data
		if (!m_nBufferCount) {
			if (!ReadNote())  // чтение ноты
			{
				 {
		...
		DWORD size = X86_Convert32To16(lpBuffer,
					 	MixSoundBuffer,
					 	lTotalSampleCount,
					 	& nVUMeterMin,
					 	& nVUMeterMax); 
	...
	if ((gnVUMeter = (UINT) (nVUMeterMax - nVUMeterMin)) > 0xFF)
		gnVUMeter = 0xFF;
	if (nStat) {
		m_nMixStat += nStat - 1; m_nMixStat /= nStat;
	}
	return lMax - lRead;
}
...
BOOL CDecompressTrack::ProcessRow()
	//---------------------------
{
	if (++m_nTickCount >= m_nMusicSpeed * (m_nPatternDelay + 1) +
		m_nFrameDelay) {
		m_nPatternDelay = 0;
		m_nFrameDelay = 0;
		m_nTickCount = 0;
		...
		if (!(m_dwSongFlags & SONG_PATTERNLOOP)) {
			...
			while (m_nPattern >= MAX_PATTERNS) {
				...
				if ((m_nPattern == 0xFF) || (m_nCurrentPattern >= MAX_ORDERS)) {
					...
					//					if (m_nRepeatCount > 0) m_nRepeatCount--; //!!!marat
...
BOOL CDecompressTrack::ReadNote()
	//-------------------------
{
	if (!ProcessRow())
		return FALSE;
	////////////////////////////////////////////////////////////////////////////////////
	m_nTotalCount++;  // общее количество тиков
	if (!m_nMusicTempo)
		return FALSE;
	m_nBufferCount = (gdwMixingFreq * 5 * m_nTempoFactor) /
		(m_nMusicTempo << 8);
	...
	DWORD nMasterVol; {
		...
		if ((m_dwSongFlags & SONG_GLOBALFADE) && (m_nGlobalFadeMaxSamples)) {
			mastervol = _muldiv(mastervol,
							m_nGlobalFadeSamples,
	...
	for (UINT nChn = 0; nChn < MAX_CHANNELS; nChn++,pChn++) {
		...
		if ((pChn->nPeriod) && (pChn->nLength)) {
			...
			if (pChn->dwFlags & CHN_TREMOLO) {
				...
				if (vol > 0) {
					...
					case 1:
						vol += (ModRampDownTable[trempos] * (int)
							pChn->nTremoloDepth) >>
							tremattn;
						break;
					case 2:
						vol += (ModSquareTable[trempos] * (int)
							pChn->nTremoloDepth) >>
							tremattn;
						break;
					case 3:
						vol += (ModRandomTable[trempos] * (int)
							pChn->nTremoloDepth) >>
							tremattn;
						break;
					default:
						vol += (ModSinusTable[trempos] * (int)
							pChn->nTremoloDepth) >>
							tremattn;
					}
				}
			...
			if (pChn->nCommand == CMD_TREMOR) {
				UINT n = (pChn->nTremorParam >> 4) +
					(pChn->nTremorParam & 0x0F);
			...
			if (pChn->pHeader) {
				...
				if ((pChn->dwFlags & CHN_VOLENV) && (penv->nVolEnv)) {
					...
					if (envpos > x2)
						envpos = x2;
					if ((x2 > x1) && (envpos > x1)) {
						envvol += ((envpos - x1) * (((int) penv->VolEnv[pt] <<
							2) - envvol)) /
							(x2 - x1);
					}
					if (envvol < 0)
			...
			if (vol) {
				...
				pChn->nRealVolume = _muldiv(vol * m_nGlobalVolume,
										pChn->nGlobalVol * pChn->nInsVol,
										1 << 20);
			}
			...
			if ((pChn->pHeader) &&
					...
					// Pitch Envelope
				{
					...
					if (l < 0) {
						...
						if (l > 255)
							l = 255;
						period = _muldiv(period,
								 	LinearSlideUpTable[l],
								 	0x10000);
					} else {
						if (l > 255)
							l = 255;
						period = _muldiv(period,
								 	LinearSlideDownTable[l],
								 	0x10000);
					}
				}
			}
		...
		if (pChn->pHeader) {
			...
			if (pChn->dwFlags & CHN_VOLENV) {
				...
				if ((penv->dwFlags & ENV_VOLSUSTAIN) &&
					...
					if (pChn->nVolEnvPosition ==
						(UINT) penv->VolPoints[penv->nVolSustainEnd] +
						1)
			...
			if (pChn->dwFlags & CHN_PANENV) {
				...
				if ((penv->dwFlags & ENV_PANSUSTAIN) &&
					(pChn->nPanEnvPosition ==
					(UINT) penv->PanPoints[penv->nPanSustainEnd] +
					1) &&
					(!(pChn->dwFlags & CHN_KEYOFF))) {
					// Panning sustained
			...
			if (pChn->dwFlags & CHN_PITCHENV) {
				...
				if ((penv->dwFlags & ENV_PITCHSUSTAIN) &&
					...
					if (pChn->nPitchEnvPosition ==
						(UINT) penv->PitchPoints[penv->nPitchSustainEnd] +
						1)
		...
		if (((pChn->nInc >> 16) + 1) >= (LONG)
			(pChn->nLoopEnd - pChn->nLoopStart))
			pChn->dwFlags &= ~CHN_LOOP;
		pChn->nNewRightVol = pChn->nNewLeftVol = 0;
		pChn->pCurrentSample = ((pChn->pSample) &&
			(pChn->nLength) &&
			(pChn->nInc)) ?
			pChn->pSample :
			NULL;
...
#endif
#ifdef MODPLUG_TRACKER
			UINT kChnMasterVol = (pChn->dwFlags& CHN_EXTRALOUD) ?
				0x100 :
				nMasterVol;
#else
#define		kChnMasterVol	nMasterVol
			...
			if (gnChannels >= 2) {
				...
				pan *= 128;//(int)m_nStereoSeparation;
				pan /= 128;
				pan += 128;
	...
	if ((m_nMixChannels >= 32) && (!(gdwSoundSetup & SNDMIX_DIRECTTODISK))) {
